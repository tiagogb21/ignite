...
var verifyUserWithSameEmailExists = async (request, reply) => {
  ...
  if (userWithSameEmail) {
    return reply.status(import_http_status2.default.CONFLICT).send({ message: error.message });
  }
};

// src/http/routes/user/register.ts
async function userRoutes(app2) {
  app2.post("/users", { preHandler: verifyUserWithSameEmailExists }, register);
}
